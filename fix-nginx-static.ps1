...
scp -i $env:USERPROFILE\.ssh\id_rsa_github -o StrictHostKeyChecking=no nginx-django-fixed.conf ubuntu@fourmindstech.com.br:/tmp/nginx-django-fixed.conf

# 2. Aplicar nova configuração
Write-Host "⚙️ Aplicando nova configuração..." -ForegroundColor Cyan
ssh -i $env:USERPROFILE\.ssh\id_rsa_github -o StrictHostKeyChecking=no ubuntu@fourmindstech.com.br "sudo cp /tmp/nginx-django-fixed.conf /etc/nginx/sites-available/django && sudo ln -sf /etc/nginx/sites-available/django /etc/nginx/sites-enabled/ && sudo nginx -t"
...
ssh -i $env:USERPROFILE\.ssh\id_rsa_github -o StrictHostKeyChecking=no ubuntu@fourmindstech.com.br "sudo mkdir -p /home/django/sistema-de-agendamento/staticfiles && sudo chown -R www-data:www-data /home/django/sistema-de-agendamento/staticfiles && sudo chmod -R 755 /home/django/sistema-de-agendamento/staticfiles"
...
ssh -i $env:USERPROFILE\.ssh\id_rsa_github -o StrictHostKeyChecking=no ubuntu@fourmindstech.com.br "sudo systemctl restart nginx"

# 6. Testar arquivos estáticos
Write-Host "🧪 Testando arquivos estáticos..." -ForegroundColor Cyan
Start-Sleep -Seconds 5
try {
    $response = Invoke-WebRequest -Uri http://fourmindstech.com.br/agendamento/static/admin/css/base.css -Method Get
