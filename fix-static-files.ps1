...
ssh -i $env:USERPROFILE\.ssh\id_rsa_github -o StrictHostKeyChecking=no ubuntu@fourmindstech.com.br "sudo -u django ls -la /home/django/sistema-de-agendamento/staticfiles/"

# 4. Corrigir permissões
Write-Host "🔐 Corrigindo permissões..." -ForegroundColor Yellow
ssh -i $env:USERPROFILE\.ssh\id_rsa_github -o StrictHostKeyChecking=no ubuntu@fourmindstech.com.br "sudo chown -R www-data:www-data /home/django/sistema-de-agendamento/staticfiles/ && sudo chmod -R 755 /home/django/sistema-de-agendamento/staticfiles/"

# 5. Verificar configuração do Nginx
Write-Host "🌐 Verificando configuração do Nginx..." -ForegroundColor Yellow
ssh -i $env:USERPROFILE\.ssh\id_rsa_github -o StrictHostKeyChecking=no ubuntu@fourmindstech.com.br "sudo cat /etc/nginx/sites-enabled/django | grep -A 5 'location /static/'"
